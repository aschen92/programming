...
updated: 10/2012 to allow asynchronous querying. See ask_nowait, response_ready,
         and getResponse.
"""

import select

PROLOG_COMMAND='swipl -G128m -f pylogic.pl -t pylogic:server -g pylogic:server 2> /dev/null'
...
class KB:
    ...
    def ask_nowait(self, query):
        """Send query to Prolog engine, and return immediately."""
    ...
    def response_ready(self):
        """ Returns True iff there is a reponse available after an
        ask_nowait query"""
        
        pipes = select.select([self.read],[],[],0)
        return pipes[0] != []


    def getResponse(self):
        """Returns response from an ask_nowait query."""
    ...
    def ask(self, query):
        """Sends query to Prolog. Returns a (possibly empty)
        dictionary of bindings if the query succeeds. Returns None if
        the query fails."""
